<template>
	<view>
		<view>
			...
			<view class="input-content" :style="{ height: inputContentHeight }">
				<textarea :style="{width: inputContentWidth}" maxlength="-1" v-model="textareaData" @blur="getCursor"></textarea>
			</view>
		</view>
		...
		methods: {
			...
			toolBarClick(type) {
				if (type == 'bold') {
					...
					this.textareaData += "<sup>上角标</sup> "
				} else if (type == "sub") {
					this.textareaData += "<sub>下角标</sub> "
			...
			let toolbar = uni.createSelectorQuery().in(this).select(".toolbar");
      		toolbar.boundingClientRect((dom) => {
				if (windowHeight !== null) {
					this.inputContentHeight = (windowHeight - dom.height) + 'px';
					this.inputContentWidth = windowWidth + 'px';
				}
      		}).exec();
